...
class Interface(object):
	...
	def __MakeWindows(self):
		...
		# Queste devono essere allineate con l'inizio degli "if" sopra
		self.wndDungeonInfo = uidungeoninfo.DungeonInfo()
		self.wndDungeonInfo.BindInterface(self)
	...
	def ToggleDungeonInfoWindow(self):
		# Aggiungiamo un feedback per essere sicuri che arrivi qui
		# chat.AppendChat(chat.CHAT_TYPE_INFO, "DEBUG: Interface riceve comando") 
		
		if self.wndDungeonInfo.IsShow():
			self.wndDungeonInfo.Close()
		else:
			self.wndDungeonInfo.Open()
			self.wndDungeonInfo.Show() # Forza la visibilità
			self.wndDungeonInfo.SetTop() # Portala in primo piano
